...
class EntityClassify(nn.Module):
    ...
    def inference(self, g, batch_size, device, num_workers, x=None):
        """Mini-batch inference of final representation over all node types.
...
class BiLSTM(nn.Module):
    def __init__(self, hidden_dim=256):
        ...
        self.distilBert_url = 'sentence-transformers/distiluse-base-multilingual-cased-v1'
        self.number_classes = 1
        self.distilBert = DistilBertModel.from_pretrained(self.distilBert_url)
        self.lstm = nn.LSTM(768, hidden_dim, batch_first=True, bidirectional=True)
        self.dropout = nn.Dropout(p=0.3)
        self.linear = nn.Linear(hidden_dim * 2, self.number_classes, bias=True)
        self.sigmoid = nn.Sigmoid()

    def forward(self, input_ids, attention_mask):
        output = self.distilBert(input_ids, attention_mask=attention_mask)[0]
        lstm_output, (last_hidden, last_cell) = self.lstm(output)
        output_hidden = th.cat((last_hidden[0], last_hidden[1]), dim=1)
        output_hidden = self.dropout(output_hidden)
        output = self.linear(output_hidden)
